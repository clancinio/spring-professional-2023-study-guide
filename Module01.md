...
- **Separation of concerns** – beans configuration is separated from beans implementation. That means only configuration class know about beans. And each of the bean implementations does not have any information that is a component, what is the scope and etc. All of this information is in configuration class.
- **Technology agnostic** – beans may not depend on concrete IoC/DI implementation – makes it easier to switch technology. That's because any of the beans have not dependency directly on the Spring framework.
- Ability to integrate Spring with external libraries. Any class of the eternal library can be a Bean. Simply create a method with @Bean annotation that returns a new object. z
